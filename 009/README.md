...
<img width="250" height="93" alt="image" src="https://github.com/user-attachments/assets/b525835e-00ea-48f7-8247-0a596d5884ef" />
<img width="342" height="93" alt="image" src="https://github.com/user-attachments/assets/02f807ca-023d-400f-90bc-89fa2ec28ca0" />
...
<img width="1024" height="672" alt="image" src="https://github.com/user-attachments/assets/3c3ce87c-bada-4bf4-ab59-3907b59004ce" />

Written by: [Tim Miller](https://dataschool.com/people/tim/)    [資料關聯說明](https://www.atlassian.com/data/sql/sql-join-types-explained-visually)
...
合併查詢是將兩張（或多張）正規化分割的資料表，透過「關聯鍵 (通常是 PK 與 FK)」重新縫合起來的技術。
  ...
  <img width="207" height="94" alt="image" src="https://github.com/user-attachments/assets/bf337e3b-a8db-4bc5-b21b-638240f79451" />
  ...
  <img width="166" height="107" alt="image" src="https://github.com/user-attachments/assets/3e4261a3-83ec-4789-b1bc-6785677f7455" />
...
子查詢就是「把一個 SELECT 的結果，當作另一個 SELECT 的條件」。當我們不知道確切數值，必須先讓資料庫算出來時，就會用到子查詢。
  ...
  <img width="148" height="58" alt="image" src="https://github.com/user-attachments/assets/12e18bd7-a918-4764-81a8-60eb1906ddfc" />
  ...
  <img width="102" height="45" alt="image" src="https://github.com/user-attachments/assets/f0f63798-f058-4b72-b5de-65440b7a9e38" />
...
與 JOIN (橫向增加欄位) 不同，`UNION` 是「直向」將兩筆結構相似的查詢結果上下疊加在一起。
  ...
  <img width="175" height="229" alt="image" src="https://github.com/user-attachments/assets/64e31b61-caa2-4fe2-8b35-3f4cd1dcbac8" />

### 4. CTE (Common Table Expression, 通用資料表運算式) 與 NULL 處理
CTE 就像是進階版的暫存表，透過 `WITH` 關鍵字，我們可以先把複雜的查詢包裝成一個有名字的虛擬表，讓後續的 SQL 更加好讀。
  ...
  <img width="183" height="58" alt="image" src="https://github.com/user-attachments/assets/26a76580-243b-4f46-af18-2d7405850f08" />
...
>
> 「我正在使用 Python Streamlit 開發『會員消費明細系統』。我的資料庫有 `customers` (會員)、`order_details` (訂單)、`products` (商品) 三張表 (同時檢附資料表給你參考)。
> 1. 請幫我寫一段 Streamlit 程式碼。首先，用 `st.selectbox` 建立一個下拉選單，讓使用者選擇會員姓名。
...
<img width="532" height="493" alt="image" src="https://github.com/user-attachments/assets/84c33261-b416-4449-a55d-697f357fe409" />
<img width="717" height="403" alt="image" src="https://github.com/user-attachments/assets/16355b15-d54b-44cd-ba3d-e9349b16cae0" />
...
請在 Spyder 開啟 `app.py`，我們將把 `customers`, `order_details`, `products` 完美縫合在網頁中！
  ...
  <img width="1766" height="665" alt="image" src="https://github.com/user-attachments/assets/7ccf3acb-c572-4878-bb62-539965ab09c6" />
...
1. **基礎關聯：** 請在 Workbench 寫一段 `INNER JOIN` SQL，將 `products` (商品) 與 `order_details` (訂單) 關聯起來，列出「商品名稱」與「被購買的數量」。
  ...
  <img width="199" height="109" alt="image" src="https://github.com/user-attachments/assets/564eab09-b3ee-4905-8cf3-9b1955d5c76d" />
...
2. **單一子查詢：** 老闆想知道誰的消費數量比平均還要高？請先寫一個子查詢算出 `AVG(qty)`，再利用它當條件，找出 `qty > 平均值` 的訂單紀錄。
  ...
  WHERE od.qty > (
    -- 這裡就是子查詢：先算出所有訂單的平均購買數量
    SELECT AVG(qty) 
    FROM order_details
  );
  ```

  <img width="198" height="76" alt="image" src="https://github.com/user-attachments/assets/0cad1d3b-884d-482c-91e0-5ee4e12300e4" />
...
3. **抓出潛水客 (`LEFT JOIN` 應用)：** 在行銷實務上，找出「註冊了卻沒消費」的會員很重要。請寫一段 `LEFT JOIN`，結合 `WHERE ... IS NULL`，找出完全沒有出現在 `order_details` 表中的會員姓名。
  ...
  <img width="76" height="61" alt="image" src="https://github.com/user-attachments/assets/729a278b-4f58-4fb6-ad81-c5529fb40355" />
...
4. **【金門特產排行榜系統】** * **挑戰要求**：我們要在網頁上做一個「全店商品總銷量排行榜」。
  ...
  # 建立新區塊標題
  st.header("🏆 全店商品總銷量排行榜")
  st.write("利用進階 CTE 語法，快速分析出全店最熱門的商品。")
  
  # 建立查詢按鈕
  if st.button("顯示前三名熱銷榜"):
      ...
      try:
          ...
          with conn.cursor() as cursor:
              ...
              # 撰寫 CTE 核心 SQL 查詢
              sql_query = """
                  -- 第一步：建立名為 ProductSales 的 CTE 虛擬表
                  ...
                  -- 第二步：將 CTE 虛擬表與 products 真實表 JOIN 起來
                  SELECT 
                      p.product_name AS '商品名稱',
                      ps.total_qty AS '總銷量'
                  FROM ProductSales ps
                  INNER JOIN products p ON ps.product_id = p.product_id
                  ORDER BY ps.total_qty DESC  -- 依照銷量由高到低排序
                  LIMIT 3;                    -- 只取前三名
              """
              ...
              if records:
                  ...
                  # 為了讓 Streamlit 的長條圖能正確將「商品名稱」作為 X 軸，我們需要將該欄位設為 Index
                  df.set_index('商品名稱', inplace=True)
                  
                  st.success("成功為老闆拉出前三名熱銷榜！")
                  
                  # 利用 st.bar_chart 畫出柱狀圖
                  st.bar_chart(df)
                  
                  # 如果也想用表格呈現精確數字，可以解除下方註解
                  # st.dataframe(df)
              else:
                  st.warning("目前沒有任何銷售紀錄可以進行統計。")
  
      except pymysql.MySQLError as e:
          st.error(f"資料庫連線或查詢發生錯誤：{e}")
  ...
  <img width="724" height="587" alt="image" src="https://github.com/user-attachments/assets/316c9b17-6c63-4346-b308-25a67a840b8c" />
