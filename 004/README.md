...
> [!WARNING]
> **大表設計的致命缺點 (資料重複與異常)：**
> 如果有 1000 個資管系學生，那「資訊管理學系」、「管理學院公文櫃」這些字眼就會在資料庫裡重複出現 1000 次！更糟的是，如果資管系換了系主任，系統必須一口氣更新 1000 筆資料，萬一漏改一筆，資料就會出現不一致的災難。
...
2. **外鍵 (Foreign Key, FK)：** 用來指向另一張表的主鍵。它就像是一個「連結標籤」，告訴系統這筆資料歸屬於哪裡。
   - 我們在 `students` 表中加入一個 `dept_id` 欄位作為 FK。當某個學生的 `dept_id` 填入 `IM` 時，系統就知道他隸屬於資管系。
